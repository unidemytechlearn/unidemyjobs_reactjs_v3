/*
  # Application Status Timeline and Visualization
  ...
  2. Changes
    - Add application status timeline table
    - Create functions to track status changes
    - Add status visualization helpers
...
CREATE OR REPLACE FUNCTION get_application_status_timeline(application_id UUID)
RETURNS TABLE (
  id UUID,
  status TEXT,
  notes TEXT,
...
CREATE OR REPLACE FUNCTION get_application_status_with_metadata(application_id UUID)
RETURNS TABLE (
  application_id UUID,
  status TEXT,
  display_name TEXT,
...
CREATE OR REPLACE FUNCTION get_application_timeline_with_metadata(application_id UUID)
RETURNS TABLE (
  timeline_id UUID,
  application_id UUID,
  status TEXT,
