/*
  ...
  2. Functions
    ...
    - Timeline retrieval functions
    - Notification functions

  3. Triggers
    - Automatic status timeline creation
...
CREATE OR REPLACE FUNCTION get_application_status_timeline(app_id UUID)
RETURNS TABLE (
  id UUID,
  status TEXT,
  notes TEXT,
...
CREATE OR REPLACE FUNCTION get_application_status_with_metadata(app_id UUID)
RETURNS TABLE (
  app_id UUID,
  status TEXT,
  display_name TEXT,
...
CREATE OR REPLACE FUNCTION get_application_timeline_with_metadata(app_id UUID)
RETURNS TABLE (
  timeline_id UUID,
  app_id UUID,
  status TEXT,
