/*
  # Add Sample Jobs and Companies for Testing

  1. New Data
    - 6 sample companies across different industries
    - 11 sample jobs with various types and experience levels
    - Realistic job descriptions, requirements, and benefits
    - Featured and regular jobs for testing

  2. Companies Added
    - TechFlow Solutions (Technology, Featured)
    - Creative Design Studio (Design, Featured) 
    - DataInsight Analytics (Data & Analytics)
    - GrowthHack Marketing (Marketing)
    - CloudScale Infrastructure (Technology, Featured)
    - FinTech Innovations (Finance)

  3. Jobs Added
    - Senior Frontend Developer (Full Time, Featured)
    - UX/UI Designer (Full Time, Featured)
    - Data Scientist (Full Time, Remote)
    - Digital Marketing Specialist (Full Time, Entry-level)
    - DevOps Engineer (Full Time, Senior, Featured)
    - Product Manager (Full Time, Mid-level)
    - Frontend Developer Intern (Internship)
    - Freelance Graphic Designer (Freelancing, Remote)
    - Part-Time Marketing Assistant (Part Time)
    - Senior Backend Engineer (Full Time, Senior, Featured)
*/
